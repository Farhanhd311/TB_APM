...
from utils.auth import init_auth_state, render_account_sidebar
...
with st.sidebar:
    ...
    st.markdown(f"<div style='font-size:0.75rem; color:#7DCFBA; padding:0 8px 8px; font-weight:600; text-transform:uppercase; letter-spacing:1px;'>{t('nav_navigasi', lang)}</div>", unsafe_allow_html=True)
...
#  METRIC CARDS
# ══════════════════════════════════════════════
st.markdown(f'<div class="section-title">{t("home_perf", lang)}</div><div class="section-sub">{t("home_perf_sub", lang)}</div>', unsafe_allow_html=True)

c1, c2, c3, c4 = st.columns(4)
with c1:
    st.metric(t("home_accuracy", lang), "99.99%", "Test Score")
with c2:
    st.metric(t("home_mae", lang), "~1.77 kcal", "Mean Abs. Error")
with c3:
    st.metric(t("home_trees", lang), "100 trees", "n_estimators")
with c4:
    total_preds = db.get_prediction_count()
    st.metric(t("home_total_pred", lang), f"{total_preds}x", "Total Database")
...
# ══════════════════════════════════════════════
st.markdown(f'<div class="section-title">{t("home_fi_title", lang)}</div><div class="section-sub">{t("home_fi_sub", lang)}</div>', unsafe_allow_html=True)
...
with col_fi:
    ...
    label_map = {
        "Duration":   t("feat_duration", lang),
        "Heart_Rate": t("feat_heartrate", lang),
        "Body_Temp":  t("feat_bodytemp", lang),
        "Weight":     t("feat_weight", lang),
        "Age":        t("feat_age", lang),
        "Height":     t("feat_height", lang),
        "Gender":     t("feat_gender", lang),
    }
...
with col_eval:
    st.markdown(f'<div style="color:#7DCFBA; font-size:0.85rem; font-weight:600; margin-bottom:10px;">{t("home_eval", lang)}</div>', unsafe_allow_html=True)
...
#  HOW TO USE
# ══════════════════════════════════════════════
st.markdown(f'<div class="section-title">{t("home_usage", lang)}</div><div class="section-sub">{t("home_usage_sub", lang)}</div>', unsafe_allow_html=True)
...
border:1px solid rgba(0,201,167,0.3); border-radius:20px; padding:36px;'>
    ...
    <div style='color:#7DCFBA; margin-bottom:20px;'>{t("home_cta_sub", lang)}</div>
</div>
""", unsafe_allow_html=True)
